...
function addInventory(prod, quantity) {
    // assign new value to product object
    prod.inventory += quantity; 
    console.log(quantity + " " + prod.name + " added to the inventory." ); 
}
...
function processSale(prod, quantity) {
    // First part
    prod.unit_price -= quantity; 
    console.log(quantity + " " + prod.name + " sold."); 
    // Second part
    totalSale = prod.unit_price * quantity; 
    // Step Five
    console.log("Total Sale: " + "$" + totalSale); 
}

processSale(product, 4); 
